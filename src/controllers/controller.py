...
class IndexController(MethodView):
    ...
    def post(self):
        ...
        with mysql.cursor() as cur:
            try:
                cur.execute(
                    "INSERT INTO sys.TAB_PACIENTES(codigoPaciente, CPF,nome,dataNascimento,idade,codigoColetaPaciente) VALUES(%s, %s, %s, %s, %s, %s)",
                    (codigoPaciente, CPF, nome, dataNascimento, idade, codigoColetaPaciente))
...
class UpdatePacienteController(MethodView):
    ...
    def post(self, code):
        ...
        with mysql.cursor() as cur:
            cur.execute(
                "UPDATE sys.TAB_PACIENTES SET CPF=%s, nome=%s, dataNascimento=%s, idade=%s,codigoColetaPaciente=%s WHERE codigoPaciente =%s",
                (CPF, nome, dataNascimento, idade, codigoColetaPaciente, code))
            cur.connection.commit()
            return redirect('/')
